...
class VariantAllele
{
 ...
 public:
  ...
  bool isIndel(void);
  */
  VariantAllele(string r, string a, long p) : ref(r), alt(a), position(p)
  {
    std::stringstream s;
    s << position << ":" << ref << "/" << alt;
    repr = s.str();
...
public:
  ...
  std::set<string> altSet(void);  // set of alleles, rather than vector of them
  ...
  std::map<std::string, std::vector<std::string> > info;  // vector<string> allows for lists by Genotypes or Alternates
  std::map<std::string, bool> infoFlags;
  VariantFieldType formatType(string& key);
  vector<string> format;
  std::map<std::string, std::map<std::string, std::vector<std::string> > > samples;  // vector<string> allows for lists by Genotypes or Alternates
  std::vector<std::string> sampleNames;
