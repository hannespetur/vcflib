...
void Variant::parse(std::string& line, bool parseSamples) {
  ...
  if (fields.size() < 7)
  {
    std::cerr << "broken VCF record (less than 7 fields)" << std::endl << line << endl;
    exit(1);
  }

  sequenceName = fields.at(0);
  sequence_name = fields.at(0);
...
VariantFieldType Variant::infoType(string& key) {
  ...
  bool Variant::getInfoValueBool(string& key, int index)
  {
    ...
    if (s == vcf->infoTypes.end()) {
      ...
      {
        std::cerr << "not flag type " << key << std::endl;
      }
    }
    return false;
  }
  ...
  bool Variant::getSampleValueBool(string& key, string& sample, int index)
  {
    ...
    else
    {
      ...
      if (index == INDEX_NONE)
      {
        if (count != 1)
        {
          std::cerr << "no field index supplied and field count != 1" << std::endl;
          exit(1);
        }
        else
        {
      ...
      {
        std::cerr << "Not bool type: " << key << std::endl;
      }
    }
    return false;
  }

  std::string Variant::getSampleValueString(string& key, string& sample, int index)
  {
    map<string, VariantFieldType>::iterator s = vcf->formatTypes.find(key);
    if (s == vcf->infoTypes.end()) {
      cerr << "no info field " << key << endl;
  ...
  double Variant::getSampleValueFloat(string& key, string& sample, int index)
  {
    ...
    if (s == vcf->infoTypes.end()) {
      ...
      {
        std::cerr << "unsupported type for sample " << type << std::endl;
      }
    }
    return 0.0;
  }
  ...
  ostream& operator<<(ostream& out, Variant& var) {
    ...
    if (length(var.sequence_name) == 0) var.sequenceName = ".";
...
map<pair<int, int>, int> Variant::getGenotypeIndexesDiploid(void) {
  ...
  vector<int> indexes;
  for (int i = 0; i < static_cast<int>(alleles.size()); ++i) {
    indexes.push_back(i);
  }
