...
void printSummary(char** argv) {
  cerr << "usage: " << argv[0] << " [options] [<vcf file>]" << endl
     ...
     << "enable use of the VCF as a DAG (specifically a partially-ordered graph)." << std::endl;
  exit(0);
}
...
int main(int argc, char** argv)
{
  ...
  while (true)
  {
    ...
    {
      case 'r':
        fastaFileName = std::string(optarg);
        break;
  ...
  if (!variantFile.is_open()) {
    cerr << "could not open VCF file" << std::endl;
    exit(1);
  }

  FastaReference reference;
  if (fastaFileName.empty()) {
    cerr << "a reference is required" << std::endl;
    exit(1);
  } else {
